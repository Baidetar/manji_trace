// ignore_for_file: constant_identifier_names, curly_braces_in_flow_control_structures, avoid_print, deprecated_member_use, no_logic_in_create_state, prefer_typing_uninitialized_variables, library_prefixes
...
class PickerWidgetState<T> extends State<_PickerWidget> {
  ...
  Widget _buildCupertinoPicker(BuildContext context, int i, int _length,
    ...
    // 修复桌面端使用滚轮滚动不精确
    // 来源：https://github.com/yangyxd/flutter_picker/issues/194
    if (!m.containsKey(i)) {
      // isMouseWheel:为了维持鼠标拖拽时的丝滑，如果不做这个处理，那么鼠标拖拽时也会每次只能跳一格
      m[i] = {
        "isMouseWheel": false,
        "indexCorrect": scrollController[i].initialItem,
      };
    }
    ...
    return Listener(
      ...
      child: CupertinoPicker.builder(
        ...
        onSelectedItemChanged: (int selectedIndex) {
          if (_length <= 0) return;
          if (m[i]["isMouseWheel"] == true) {
            if (selectedIndex > m[i]["indexCorrect"]) {
              m[i]["indexCorrect"] = m[i]["indexCorrect"] + 1;
              scrollController[i].jumpToItem(m[i]["indexCorrect"]);
            } else if (selectedIndex < m[i]["indexCorrect"]) {
              scrollController[i].jumpToItem(--m[i]["indexCorrect"]);
            }
            m[i]["isMouseWheel"] = false;
          } else {
            m[i]["indexCorrect"] = selectedIndex;
          }
          var index = m[i]["indexCorrect"] % _length;
